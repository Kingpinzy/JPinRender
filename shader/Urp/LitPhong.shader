    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            half4 frag(Varyings input) : SV_Target
            {
                ...
                float3 reflectDir = normalize(reflect(-mainLight.direction, normal));//反射方向
                // float3 hav = normalize(mainLight.direction + viewDir);
                float vr = saturate(dot(reflectDir,viewDir)) ;
                float3 specular =  _SpecularColor * pow(vr, _Gloss);//镜面反射

                //---加在一起
                half4 output;
                float3 finaColor = diffuse + specular ;
